...
import { extractScenes, formatSceneHeaderWithNumber } from '../utils/sceneNumbering'
...
const ProfessionalVerticalStackEditor = forwardRef<ScriptEditorRef, ProfessionalVerticalStackEditorProps>(
  ({ value, onChange, isPreviewMode, className = '', placeholder }, ref) => {
    ...
    // Initialize with default content or empty line with scene numbering
    useEffect(() => {
      if (value && value.trim()) {
        const textLines = value.split('\n')
        const scenes = extractScenes(value)
        ...
        const scriptLines: ScriptLine[] = textLines.map((text, index) => ({
          ...
          isActive: index === 0,
          sceneNumber: sceneMap.get(index)
        }))
    ...
    const detectFormat = useCallback((text: string): string => {
      ...
      // Transition detection - broader pattern
      if (/^(CUT TO:|FADE TO:|DISSOLVE TO:|MATCH CUT:|SMASH CUT:|CUT TO|FADE TO|DISSOLVE TO)(.*)$/i.test(trimmed)) {
        return 'Transition'
      }
    ...
    useImperativeHandle(ref, () => ({
      formatCurrentLine: (format: string) => {
        ...
        // Auto-focus the current line after formatting
        setTimeout(() => {
          const activeRef = lineRefs.current[lines[activeLineIndex]?.id]
          if (activeRef) {
            activeRef.focus()
            // Place cursor at end of text
            activeRef.setSelectionRange(activeRef.value.length, activeRef.value.length)
          }
        }, 50)
      },
      ...
      getFormattedLines: () => lines
    }))

    // Handle line content changes with scene number updates
    const handleLineChange = useCallback((lineId: string, newText: string) => {
      setLines(prevLines => {
        ...
        // Update scene numbers after content change
        const allText = linesToText(newLines)
        const scenes = extractScenes(allText)
        const sceneMap = new Map<number, number>()
        scenes.forEach(scene => {
          sceneMap.set(scene.lineIndex, scene.sceneNumber)
        })
        
        // Apply scene numbers to lines
        const numberedLines = newLines.map((line, index) => ({
          ...line,
          sceneNumber: sceneMap.get(index)
        }))
        ...
        setTimeout(() => {
          onChange(linesToText(numberedLines))
        }, 0)
    ...
    return (
      <div className={`flex-1 flex flex-col ${className} relative`} ref={containerRef}>
        ...
        {/* Vertical Line Stack */}
        <div className="flex-1 w-full p-8 pl-16 overflow-y-auto bg-white">
          <div className="max-w-4xl mx-auto space-y-1 relative">
            {lines.map((line, index) => (
              <div key={line.id} className="relative">
                ...
                {/* Line number and scene number indicators */}
                {line.isActive && (
                ...
                {/* Scene number - ROOT CAUSE FIX */}
                {line.format === 'SceneHeading' && line.text.trim() && (
                  <div 
                    className="absolute top-0 w-10 h-6 bg-blue-600 text-white font-bold text-xs flex items-center justify-center rounded shadow-md z-20"
                    style={{
                      left: '-52px',
                  ...
                  >
                    {(() => {
                      ...
                      for (let i = 0; i < index; i++) {
                        if (lines[i].format === 'SceneHeading' && lines[i].text.trim()) {
                          sceneCount++
                        }
                      }
                      return sceneCount
